...
//        $transaction->save();
    ...
    if (Auth::user()->role != 3 || Auth::user()->admin_status != 1 || $request->segment != 0) {
        $incomeQuery->where('project_id', Auth::user()->project_id);
        $expenseQuery->where('project_id', Auth::user()->project_id);
        if ($request->segment != 0) {
            $incomeQuery->where('product_segment_id', $request->segment);
            $expenseQuery->where('product_segment_id', $request->segment);
        }
    }

    $start_date = $request->filled('start') ? date("Y-m-d", strtotime($request->start)) : null;
    $end_date = $request->filled('end') ? date("Y-m-d", strtotime($request->end)) : null;

    if ($start_date && $end_date) {
        $incomeQuery->whereBetween('date', [$start_date, $end_date]);
        $expenseQuery->whereBetween('date', [$start_date, $end_date]);
    }

    $incomeQuery->where('transaction_type', 1)
        ->select(DB::raw('sum(amount) as amount, account_head_type_id'))
        ->where('account_head_type_id', '!=', 0);
