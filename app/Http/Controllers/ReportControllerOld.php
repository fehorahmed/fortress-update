...
class ReportController extends Controller
{
    ...
    public function purchaseReport(Request $request){
        ...
        return view('report.purchase_report', compact('orders', 'suppliers',
            'products', 'appends','projects'))->with($data);
    }
...
//            $contractorPayments= Contractor::where('project_id',$request->project)->get();
        ...
        return view('report.project_report',compact('projects','supplierPayments','contractorPayments','otherIncomes','incomes','start','end',
    ...
    public function cashStatement(Request $request) {
        ...
        $end_date = date("Y-m-d", strtotime($request->end));
            ...
            if($request->segment != 0){
                ...
                $transactionLogs = TransactionLog::where('transaction_method', 1)
                    ->get();
            }
...
//    }
    ...
    if ($start_date && $end_date && $request->segment != 0) {
        $incomes = TransactionLog::whereIn('transaction_type', [1])
            ->where('project_id',Auth::user()->project_id)
            ->whereIn('product_segment_id',[$request->segment])
            ->with('transaction')
    ...
    }else if ($start_date == '' && $end_date =='' && $request->segment != 0 ) {
        $incomes = TransactionLog::whereIn('transaction_type', [1])
            ->where('project_id', Auth::user()->project_id)
            ->whereIn('product_segment_id', [$request->segment])
            ->with('transaction')
