        ...
        <div class="col-md-12">
            <div class="card card-outline card-primary">
                <div class="card-header with-border">
                    <h3 class="card-title">StakeHolder Payment Information</h3>
                </div>

                <form class="form-horizontal" method="POST" enctype="multipart/form-data" action="{{ route('stakeholder.payment_receipt.edit',['stakeholderPayment' => $stakeholderPayment->id]) }}">
                    @csrf
                    ...
                    <div class="card-body">
                        <div class="form-group row {{ $errors->has('name') ? 'has-error' :'' }}">
                            ...
                            <div class="col-sm-10">
                                <input type="text" class="form-control" name="name" readonly
                                        value="{{ old('name',$stakeholderPayment->stakeholder->name??'') }}">
                                @error('name')
                                <span class="help-block">{{ $message }}</span>
                                @enderror
                            </div>
                        </div>
                        <div class="form-group row {{ $errors->has('stakeholder_due') ? 'has-error' :'' }}">
                            <label class="col-sm-2 control-label">StakeHolder Due *</label>
                            <div class="col-sm-10">
                                <input type="text" class="form-control" name="stakeholder_due" readonly
                                       value="{{ old('stakeholder_due',$stakeholderPayment->stakeholder->due??'') }}">
                                @error('stakeholder_due')
                                <span class="help-block">{{ $message }}</span>
                                @enderror
                            </div>
                        </div>
                            ...
                            <div class="col-sm-10">
                                <select class="form-control" name="segment" id="segment" required>
                                    ...
                                    @foreach($segments as $segment)
                                        <option value="{{ $segment->id }}" {{ old('segment',$stakeholderPayment->product_segment_id) == $segment->id ? 'selected' : '' }}>{{ $segment->name }}</option>
                                    @endforeach
                                </select>
                                @error('segment')
                                <span class="help-block">{{ $message }}</span>
                        ...
                        <div class="form-group row {{ $errors->has('payment_type') ? 'has-error' :'' }}">
                            ...
                            <div class="col-sm-10">
                                <select class="form-control" name="payment_type" id="payment_type" required>
                                    <option value="1" {{ old('payment_type',$stakeholderPayment->transaction_method) == '1' ? 'selected' : '' }}>Cash</option>
                                    <option value="2" {{ old('payment_type',$stakeholderPayment->transaction_method) == '2' ? 'selected' : '' }}>Bank</option>
                                </select>
                                @error('payment_type')
                                <span class="help-block">{{ $message }}</span>
                                @enderror
                        ...
                        <div id="bank-info">
                                ...
                                <div class="col-sm-10">
                                    <select class="form-control" name="account" id="account">
                                        ...
                                        @foreach($bankAccounts as $bankAccount)
                                            <option value="{{ $bankAccount->id }}" {{ old('account',$stakeholderPayment->bank_account_id) == $bankAccount->id ? 'selected' : '' }}>{{ $bankAccount->bank->name }}-{{ $bankAccount->branch->name }}-{{ $bankAccount->account_no }}</option>
                                        @endforeach
                                    </select>
                                    @error('account')
                                    <span class="help-block">{{ $message }}</span>
                            ...
                            <div class="form-group row {{ $errors->has('cheque_no') ? 'has-error' :'' }}">
                                ...
                                <div class="col-sm-10">
                                    <input type="text" class="form-control" value="{{ old('cheque_no',$stakeholderPayment->cheque_no) }}" name="cheque_no" placeholder="Enter Cheque No.">
                                    @error('cheque_no')
                                    <span class="help-block">{{ $message }}</span>
                                    @enderror
                                </div>
                                ...
                                <div class="col-sm-4">
                                    <div class="input-group date">
                                        <input type="text" class="form-control pull-right" id="cheque_date"
                                               name="cheque_date" value="{{old('Y-m-d',date('Y-m-d',strtotime($stakeholderPayment->cheque_date)))}}" autocomplete="off">
                                    </div>
                                    @error('cheque_date')
                                    <span class="help-block">{{ $message }}</span>
                                    @enderror
                        ...
                        <div class="form-group row {{ $errors->has('amount') ? 'has-error' :'' }}">
                            ...
                            <div class="col-sm-10">
                                <input type="text" class="form-control" name="amount" placeholder="Enter Amount" value="{{ old('amount',$stakeholderPayment->total) }}">
                                @error('amount')
                                <span class="help-block">{{ $message }}</span>
                                @enderror
                            </div>
                        ...
                        <div class="form-group row {{ $errors->has('date') ? 'has-error' :'' }}">
                            ...
                            <div class="col-sm-4">
                                <div class="input-group date">
                                    <input type="text" class="form-control pull-right" id="date" name="date" value="{{old('Y-m-d',date('Y-m-d',strtotime($stakeholderPayment->date)))}}" autocomplete="off">
                                </div>
                                @error('date')
                                <span class="help-block">{{ $message }}</span>
                                @enderror
                        ...
                        <div class="form-group row {{ $errors->has('note') ? 'has-error' :'' }}">
                            ...
                            <div class="col-sm-10">
                                <input type="text" class="form-control" name="note" placeholder="Enter Note" value="{{ old('note',$stakeholderPayment->note) }}">
                                @error('note')
                                <span class="help-block">{{ $message }}</span>
                                @enderror
                            </div>
...
@section('script')
    ...
    <script>
        $(function () {
            var accountHeadTypeSelected = '{{ old('account_head_type',$stakeholderPayment->account_head_type_id) }}';
            var accountHeadSubTypeSelected = '{{ old('account_head_sub_type',$stakeholderPayment->account_head_sub_type_id) }}';
