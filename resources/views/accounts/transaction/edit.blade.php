    ...
    <div class="row">
        <div class="col-md-12">
            <div class="box">
                ...
                <!-- /.box-header -->
                <!-- form start -->
                <form class="form-horizontal" method="POST" enctype="multipart/form-data" action="{{ route('transaction.edit', $transaction->id) }}">
                    @csrf
                    ...
                    <div class="box-body">
                        <div class="form-group {{ $errors->has('type') ? 'has-error' :'' }}">
                            ...
                            <div class="col-sm-10">
                                <select class="form-control" name="type" id="type">
                                    <option selected value="{{ $transaction->transaction_type }}">
                                        @if ($transaction->transaction_type == 1) Income
                                        @elseif($transaction->transaction_type == 2) Expense    
                                        @endif
                                    </option>
                        ...
                        <div class="form-group {{ $errors->has('account_head_type') ? 'has-error' :'' }}">
                            <label class="col-sm-2 control-label">Group Summary Name *</label>

                            <div class="col-sm-10">
                                <select class="form-control" name="account_head_type" id="account_head_type">
                                    <option selected value="{{ $transaction->account_head_type_id }}"> {{ $transaction->accountHeadType->name??'' }} </option>
                                </select>
                        ...
                        <div class="form-group {{ $errors->has('account_head_sub_type') ? 'has-error' :'' }}">
                            <label class="col-sm-2 control-label">Group Summary Sub Name *</label>

                            <div class="col-sm-10">
                                <select class="form-control" name="account_head_sub_type" id="account_head_sub_type">
                                    <option value="">Select Group Summary Sub Name</option>
                                </select>
                        ...
                        <div class="form-group {{ $errors->has('payment_type') ? 'has-error' :'' }}">
                            ...
                            <div class="col-sm-10">
                                <select class="form-control" name="payment_type" id="payment_type">
                                    <option selected value="{{ $transaction->transaction_method }}">
                                        @if ($transaction->transaction_method == 1 ) Cash
                                        @elseif($transaction->transaction_method == 2 ) Bank   
                                        @elseif($transaction->transaction_method == 3 ) Mobile Banking   
                                        @endif
                                    </option>
                        ...
                        <div id="bank-info">
                            <div class="form-group {{ $errors->has('bank') ? 'has-error' :'' }}">
                                ...
                                <div class="col-sm-10">
                                    <select class="form-control" name="bank" id="bank">
                                        <option selected value="{{ $transaction->bank_id }}"> {{ $transaction->bank->name??'' }} </option>
                            ...
                            <div class="form-group {{ $errors->has('branch') ? 'has-error' :'' }}">
                                ...
                                <div class="col-sm-10">
                                    <select class="form-control" name="branch" id="branch">
                                        <option selected value="{{ $transaction->branch->id??'' }}"> {{ $transaction->branch->name??'' }} </option>
                                    </select>
                            ...
                            <div class="form-group {{ $errors->has('cheque_no') ? 'has-error' :'' }}">
                                ...
                                <div class="col-sm-10">
                                    <input type="text" value="{{ old('cheque_no',$transaction->cheque_no) }}" class="form-control" name="cheque_no" placeholder="Enter Cheque No.">
                            ...
                            <div class="form-group {{ $errors->has('cheque_image') ? 'has-error' :'' }}">
                                ...
                                <div class="col-sm-10">
                                    ...
                                    <a href="{{ url('/public/'.$transaction->cheque_image) }}" target="_blank"> Show Previous Image </a>
                            ...
                            <div class="form-group {{ $errors->has('cheque_date') ? 'has-error' :'' }}">
                                ...
                                <div class="col-sm-10">
                                    <div class="input-group date">
                                        ...
                                        <input type="text" class="form-control pull-right" id="cheque_date" name="cheque_date" value="{{ empty(old('date')) ? ($errors->has('cheque_date') ? '' : date('Y-m-d', strtotime($transaction->cheque_date))) : old('cheque_date', date('Y-m-d', strtotime($transaction->cheque_date))) }}" autocomplete="off">
                                    </div>
                                    <!-- /.input group -->
                        ...
                        <div class="form-group {{ $errors->has('date') ? 'has-error' :'' }}">
                            ...
                            <div class="col-sm-10">
                                <div class="input-group date">
                                    ...
                                    <input type="text" class="form-control pull-right" id="date" name="date" value="{{ empty(old('date')) ? ($errors->has('date') ? '' : date('Y-m-d', strtotime($transaction->date))) : old('date', date('Y-m-d', strtotime($transaction->date))) }}" autocomplete="off">
                                </div>
                                <!-- /.input group -->
    ...
    <script>
        $(function () {
            ...
            $('#type').change(function () {
                ...
                $('#account_head_type').html('<option value="{{ $transaction->accountHeadType->id??"" }}"> {{ $transaction->accountHeadType->name??"" }} </option>');
                $('#account_head_sub_type').html('<option value="">Select Group Summary Sub Name</option>');
            ...
            $('#account_head_type').change(function () {
                ...
                $('#account_head_sub_type').html('<option value="{{ $transaction->accountHeadSubType->id??"" }}"> {{ $transaction->accountHeadSubType->name??"" }} </option>');
            ...
            $('#bank').change(function () {
                var bankId = $(this).val();
                $('#branch').html('<option value="{{ $transaction->branch->id??'' }}">{{ $transaction->branch->name??'' }}</option>');
            ...
            $('#branch').change(function () {
                ...
                $('#account').html('<option value="{{ $transaction->account->id??'' }}">{{ $transaction->account->account_no??'' }}</option>');
