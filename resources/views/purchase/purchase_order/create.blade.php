                        ...
                        <div class="row">
                                ...
                                <div class="form-group {{ $errors->has('segment') ? 'has-error' : '' }}">
                                    ...
                                    <select id="segment" class="form-control select2 segment" style="width: 100%;"
                                        ...
                                        @foreach ($segments as $segment)
                                            <option {{ empty(old('$segment')) ? '' : 'selected' }}
                                                value="{{ $segment->id }}"
                            ...
                            {{-- <div class="col-md-3">
                                <div class="form-group {{ $errors->has('requisition_id') ? 'has-error' : '' }}">
                                    ...
                                    <select id="requisition_id" class="form-control select2 requisition_id" style="width: 100%;"
                                        name="requisition_id" data-placeholder="Select Requisition ID">
                                                    ...
                                                    <tr class="product-item">
                                                        ...
                                                        {{-- <td>
                                                            <div
                                                                ...
                                                                <input type="number" step="any"
                                                                    class="form-control available" name="available[]"
                                                                    value="{{ old('available.' . $loop->index) }}" readonly>
                                                            </div>
                                                        </td> --}}
                                                        <td>
                                                            <div
                                        ...
                                        <td>
                                            <div class="form-group {{ $errors->has('vat') ? 'has-error' : '' }}">
                                                <input type="text" class="form-control" name="vat" id="vat"
                                                    value="{{ empty(old('vat')) ? ($errors->has('vat') ? '' : '0') : old('vat') }}">
                                                <span id="vat_total">৳0.00</span>
                                                <input type="hidden" class="form-control" name="total_vat" id="total_vat"
                                                    value="{{ empty(old('total_vat')) ? ($errors->has('total_vat') ? '' : '0') : old('total_vat') }}">
                                            </div>
                                        </td>
                                    </tr>
                                    <tr>
...
@section('script')
    <script>
        $(function() {
            ...
            // select Segment
            var selectedSegment = '{{ old('requisition_id') }}'

            $('body').on('change', '#segment', function() {
                var segmentId = $(this).val();
                $('#requisition_id').html('<option value="">Select Requisition ID</option>');
                if (segmentId != '') {
                    $.ajax({
                        method: "GET",
                        url: "{{ route('get_requisition_id') }}",
                        data: {
                            segmentId: segmentId
                        }
                    }).done(function(data) {
                        $.each(data, function(index, item) {
                            if (selectedSegment == item.id)
                                $('#requisition_id').append('<option value="' + item.id +
                                    '" selected>' + item.id_no + '</option>');
                            else
                                $('#requisition_id').append('<option value="' + item.id + '">' +
                                    item.id_no + '</option>');
                        });
            ...
            $('body').on('change', '.product', function() {
              ...
              //  $('#segment').html('<option value="">Select</option>');
                if (productId != '') {
                    ...
                    $.ajax({
                        method: "GET",
                        url: "{{ route('get_requisition_products') }}",
                        data: {
                            segmentId: segmentId,
                            productId: productId
                        }
                        ...
                        itemProduct.closest('tr').find('.available').val(data);
                    });
